matR_batch_dl.v2 <- function(
                          mgid_list,    # file with list of IDs - no header
                          include_full_annotation_string=TRUE,
                          full_annotation_sep="*",
                          list_is_file=TRUE,
  ...
  for (batch_count in start_batch:(num_batch)){
    ...
    if( this_is_first_batch==TRUE ){
      ...
      # use the full annotation string with designated sep if option is selected
      if( include_full_annotation_string==TRUE ){ rownames(my_data) <- rownames(first_batch, view="count", sep=full_annotation_sep)  }
      ...
      next_batch.matrix <- next_batch$count
      # use the full annotation string with designated sep if option is selected
      if( include_full_annotation_string==TRUE ){ rownames(next_batch.matrix) <- rownames(next_batch, view="count", sep=full_annotation_sep)  }
      
      # Add the next batch to my_data
      #my_data <- merge(my_data, data.matrix(next_batch.matrix), by="row.names", all=TRUE) # This does not handle metadata yet
  ...
  if ( batch_remainder > 0 ){ 
    ...
    last_batch.matrix <- last_batch$count
    # use the full annotation string with designated sep if option is selected
    if( include_full_annotation_string==TRUE ){ rownames(last_batch.matrix) <- rownames(last_batch, view="count", sep=full_annotation_sep)  }
    
    
    # Add the next batch to my_data
    my_data <- merge(my_data, data.matrix(last_batch.matrix), by="row.names", all=TRUE) # This does not handle metadata yet
