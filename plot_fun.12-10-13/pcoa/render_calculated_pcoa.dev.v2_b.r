  ...
  
{
  ...
  eigen_vetors <- my_data$eigen_vectors
  ...
  if ( identical( is.na(amethst_groups), FALSE ) ){ # create names for the output files
    ...
    #pcoa_colors <- as.list(color_column) #[,1, drop=FALSE]
    pcoa_colors <- color_column
  ...
  if ( identical( is.na(metadata_table), FALSE ) ){
    ...
    if ( use_all_metadata_columns==TRUE ){ # AUTOGENERATE PLOTS FOR ALL COLUMNS IN THE METADATA FILE - ONE PLOT PER METADATA COLUMN
      ...
      pcoa_colors <- as.list(color_column) # convert colors to a list after they've been used to sort the eigen vectors


       if( debug==TRUE ){
        my.plot_pch <<- plot_pch
        my.eigen_values <<- eigen_values
        my.eigen_vectors <<- eigen_vectors
        my.metadata_matrix <<- metadata_matrix
        my.metadata_column_index <<- metadata_column_index
        my.metadata_column <<- metadata_column 
        my.color_column <<- color_column
        my.ncol.color_matrix <<- ncol.color_matrix
        my.column_factors <<- column_factors
        my.column_levels <<- column_levels
        my.num_levels <<- num_levels
        my.color_levels <<- color_levels
        my.pcoa_colors <<- pcoa_colors
        my.image_out <<- image_out
        my.figure_main <<- figure_main
      }

      
      # if( debug==TRUE ){ sorted_vectors <<- eigen_vectors }
