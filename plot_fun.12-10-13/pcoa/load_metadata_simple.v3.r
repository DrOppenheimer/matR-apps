load_metadata_simple.v3 <- function(metadata_table){
...
create_colors <- function(color_matrix, color_mode = "auto"){ # function to     
  ...
  for ( i in 1:dim(color_matrix)[2] ){
    ...
    color_levels <- col.wheel(num_levels, my_cex=0.75, my_pallete=the_pallete)
...
######################
col.wheel <- function(num_col, my_cex=0.75, my_pallete="rainbow") { # generate colors from rainbow -- or just use supplied list

  if( identical(my_pallete, "rainbow") ){
