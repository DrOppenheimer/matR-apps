# Change from v12 -- allow use of list of colors as the pallete
# known bug - R^2 values don't always appear correctly
...
render_pcoa.v13 <- function(
                            ...
                            custom_pallete="default", # use a custom color pallete -- enter FALSE or the list of colors to use 
  ...
  if ( identical( is.na(metadata_table), FALSE ) ){
    ...
    if ( use_all_metadata_columns==TRUE ){ # AUTOGENERATE PLOTS FOR ALL COLUMNS IN THE METADATA FILE - ONE PLOT PER METADATA COLUMN
      ...
      for (i in 1:ncol.color_matrix){ # loop to process through all columns
        ...
        color_column <- create_colors(metadata_column, custom_pallete, debug) # set parameters for plotting
        ncol.color_matrix <- 1 
