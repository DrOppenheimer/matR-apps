...
norm_center_scale <<- function(file_in, file_out="default") # end inputarguments
{
  ...
  if ( nargs() == 0 ){
    writeLines("
     ...
     DESCRIPTION: (norm_center_scale.from_file.r):
     ...
     This version takes a properly formatted tab delimted text as input, and generates the same.
     Output is
...
###### assign default output name if none other is given  
  if ( identical(file_out, "default") ){
    file_out <- paste(file_in, ".normalized.txt", sep="", collapse="")
  }
###### load the data  
